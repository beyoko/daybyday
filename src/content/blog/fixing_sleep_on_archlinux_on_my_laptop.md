---
title: 'Fixing sleep on Archlinux on my laptop'
pubDate: '01/20/2025'
draft: false
tags:
  - 'fix'
...
我这边是在 `/etc/default/grub` 里面的 `GRUB_LINUX_CMDLINE_DEFAULT `后面加了这两个东西，然后用 `grub-mkconfig -o /boot/grub/grub.cfg` 重新生成了 `grub.cfg`。具体操作可能每个发行版不太一样。
